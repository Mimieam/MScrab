        ...
        $(divs).droppable({
            ...
            drop: function(event, ui) {
                ...
                if ($.inArray(ui.draggable, client.usedChip) == -1){
                	...
                	//client.usedTile.push();
                }
           //      var lN = client.getNeighborCoordinate('left',client.usedChip[client.usedChip.length -1 ]);
        		 // console.log(lN);
           //       console.log (client.getNeighborCoordinate('right',ui.draggable));
           //       console.log (client.getNeighborCoordinate('top',ui.draggable));
           //       console.log (client.getNeighborCoordinate('bottom',ui.draggable));
...
function ChipHolder() {
	...
	this.getChips = function( divName) {
		...
		for (x in eqSign) {
			$(divName).append(eqSign[x].GetSelf()[0]);
		};
	};
...
var Client = function (name,homeTile) {
	...
	function getNeighbor (orientation,element){
	...
	function traverseUsedChip (direction,orientation,chip) {
		var neighbor = chip;
		...
		if (neighbor[0] == undefined)
		...
		else
			{
				if (direction == "horizontal"){
					...
					//traverseUsedChip (direction, getNeighbor("right", chip));			
				}
				else{
					if (direction == "vertical") {
						console.log("VV "+ neighbor.html());
						traverseUsedChip (direction, orientation,getNeighbor(orientation, chip));
					};
				}
...
/* we are going from the last chip*/
	this.validate = function() {
		...
		if (this.usedChip.length == 0)
			...
			var direction = ""
			var LN = getNeighbor('left',(this.usedChip[this.usedChip.length -1 ]))	||null;//all available neighbor for the last chip
			var RN = getNeighbor('right',(this.usedChip[this.usedChip.length -1 ]))	||null;
			var TN = getNeighbor('top',(this.usedChip[this.usedChip.length -1 ]))	||null;
			var BN = getNeighbor('bottom',(this.usedChip[this.usedChip.length -1 ]))||null;
			
			// console.log("has a R or L "+ (LN.length == 0 ? (RN.length == 0 ? false : true) : true ));
			// console.log("has a T or B "+ (TN.length == 0 ? (BN.length == 0 ? false : true) : true ));


			if ((LN.length == 0 ? (RN.length == 0 ? false : true) : true )){

				direction = "horizontal";
				traverseUsedChip (direction,"left",LN);
				console.log("LL "+ $(this.usedChip[this.usedChip.length -1 ]).html());
				traverseUsedChip (direction,"right",RN);
			}
